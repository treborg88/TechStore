...
| **Branch** | `feat/saas-multitenant` (active), `main` (stable) |
...
```
- **Multi-tenant**: Each store = `tenant_{slug}` PostgreSQL schema. Platform data in `public` schema.
...
| **Settings** | `app_settings` key-value per tenant — colors, fonts, layout, landing page, payments |

## 4. Technical Conventions
- **API**: `apiFetch(apiUrl('/path'))` auto-attaches auth + base URL. Backend uses raw SQL with `pg` parameterized queries (`$1, $2`).
...
| `app_settings` key-value | Add settings without schema migration |
...
Both follow: horizontal nav → sidebar controls + live preview. Prefix `lp-` (landing) vs `sc-` (site). Landing edits `landingPageConfig` JSON; Site edits flat `app_settings`.
...
- Backend stores `categoryFiltersConfig` as JSON string in `app_settings`
...
| Task | Steps |
|------|-------|
| **Add a setting** | 1) Default in SettingsManager state, 2) `else if` in `buildTypedData`, 3) Field component, 4) Include in `handleSave` payload |
...
| **Add Super Admin feature** | 1) Component in `pages/superadmin/`, 2) Add to `NAV_SYSTEM` array, 3) Render on `activeNav === 'id'`, 4) Wire QuickActions |
| **Remove endpoint cleanly** | 1) Remove route, 2) Check `routes/index.js`, 3) Search frontend for callers, 4) Remove imports/components, 5) Remove nav entries |
...
- **`default_server`** added to main nginx block — prevents Adminer block from catching `admin.eonsclover.com`.
- **`database` added to SYSTEM_SLUGS** in both frontend (`config.js`) and backend (`tenant.js`) for consistency.
