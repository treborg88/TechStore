...
app.post('/api/auth/logout', authenticateToken, (req, res) => {
    ...
    // Add token to blacklist until it expires (24 hours from now)
    const TOKEN_EXPIRY_MS = 24 * 60 * 60 * 1000; // 24 hours
    tokenBlacklist.set(req.token, Date.now() + TOKEN_EXPIRY_MS);
...
app.post('/api/products/:id/images', authenticateToken, upload.array('images', 10), async (req, res) => {
    // Only admin can add product images
    if (req.user.role !== 'admin') {
        return res.status(403).json({ message: 'Acceso denegado' });
    }
