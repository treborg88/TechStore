...
router.post('/configure', async (req, res) => {
  ...
  // Derive DATABASE_URL from Supabase creds (so backup/restore works without requiring a separate env var)
  const derivedDbUrl = await getSupabaseDerivedDatabaseUrl({ supabaseUrl: url, supabaseKey: key });
  ...
  if (derivedDbUrl) {
    envLines.push(`DATABASE_URL=${derivedDbUrl}`);
  }
  ...
  if (derivedDbUrl) {
    process.env.DATABASE_URL = derivedDbUrl;
  }
